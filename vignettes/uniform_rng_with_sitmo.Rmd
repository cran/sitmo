...
F_U\left({u}\right) &= P\left( {U \le u} \right) \\
   ...
   &= P\left( {U \le F_X^{ - 1}\left( u \right)} \right) \\
...
1. Through the use of `Rcpp`'s hook into the `Rmath.h` library that controls random generation via `R::runif(a,b)` or Rcpp's sugar `Rcpp::runif(n,a,b)`. 
2. By using C++11's [built-in generators and statistical distributions](http://en.cppreference.com/w/cpp/numeric/random) to create a uniform real random variable generator.
...
3. The ability to scale the randomly generated number between $U[a,b]$.
